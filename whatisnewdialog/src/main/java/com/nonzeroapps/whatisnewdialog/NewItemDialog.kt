...
class NewItemDialog private constructor(private val mContext: Context) {
    ...
    fun setUsePaletteForDescBackground(usePaletteForDescBackground: Boolean): NewItemDialog {
        mDialogSettings.usePaletteForDescBackground = usePaletteForDescBackground
        return this
    }

    fun setUsePaletteForImageBackground(usePaletteForImageBackground: Boolean): NewItemDialog {
        mDialogSettings.usePaletteForImageBackground = usePaletteForImageBackground
        return this
    }
    ...
    val isConditionsSuitable: Boolean
        get() = !SharedPrefHelper.isSeenBefore(mContext, mDialogSettings.getVersionName(mContext))
    ...
    fun showDialog(activity: AppCompatActivity) {
        try {
            if (activity.supportFragmentManager.findFragmentByTag(DIALOG_TAG) == null) {
                ...
                // Use commit() instead of commitAllowingStateLoss() unless necessary
                transaction.commit()
            }
    ...
    companion object {
        ...
        fun init(context: Context): NewItemDialog {
            // Use the application context: this instance is held in a
            // process-wide singleton, so caching an Activity context here would
            // leak it. Only SharedPreferences access is needed, for which the
